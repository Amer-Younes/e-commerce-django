    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.action === "added") {
                $(`.wishlist-icon${id}`).removeClass("fa-regular").addClass("fa-solid");
                $(`.Wishlist-label${id}`).attr("aria-label", "Remove From Wishlist");
                $(`.wishlist-items-count`).text(response.wishlist_count);
                console.log(response.wishlist_count);
                
            } else {
                $(`.wishlist-icon${id}`).removeClass("fa-solid").addClass("fa-regular");
                $(`.Wishlist-label${id}`).attr("aria-label", "Add To Wishlist");
                $(`.wishlist-items-count`).text(response.wishlist_count);
                console.log(response.wishlist_count);
            }
        }
...
$(document).on("click", ".delete-product-wishlist", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            this_val.closest("tr").remove(); // Remove the item from the table
            $(".wishlist-items-count").text(response.wishlist_count); // Update count
            $("#wishlist-list").html(response.data); // Refresh wishlist UI
        }
    });
});
...
$("#contact-form-ajax").submit(function (e){
    ...
    $.ajax({
        ...
        dataType : "json",
        beforeSend : function(){
            this_val.find("button").html("Sending Message ...")
        },
        success : function(response){
            
            this_val.find("button").html("Message Sent")
